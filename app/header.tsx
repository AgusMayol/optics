...
} from "@/registry/optics/theme-switcher";
...
import type { LayoutLinkGroup } from "./layout-content";
...
export function Header({ links }: { links: LayoutLinkGroup[] }) {
	const [themeSwitch, setThemeSwitch] =
		React.useState<ThemeSwitcherValue>("system");
	...
	const showMobileNav = mounted && isMobile === true;
	const showDesktopBrandMenu = mounted && isMobile === false;
	...
	const handleSetTheme = (newTheme: ThemeSwitcherValue) => {
	...
	React.useEffect(() => {
		if (!mounted) {
			const tema =
				(localStorage.getItem("theme2") as ThemeSwitcherValue | null) ||
				"system";
			setThemeSwitch(tema);
			setMounted(true);
		}
	...
	const brandLink = (
		<Link
			ref={desktopWidthRef as React.Ref<HTMLAnchorElement>}
			href="/"
			...
			{showMobileNav ? (
				<Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
					<SheetTrigger
						render={
							<Button
								ref={mobileWidthRef as React.Ref<HTMLAnchorElement>}
								variant="ghost"
								size="lg"
								animation="none"
				...
				<div className="col-span-3 border-r hidden lg:block">{brandLink}</div>
			)}
