...
type ConvertibleColorFormat = Exclude<ColorFormat, "classname" | "var">;
...
export default function Page() {
	...
	const getColorValue = (colorClass: string, format: ColorFormat) => {
		if (format === "classname") {
			return colorClass;
		}

		const colorData = tailwindColors[colorClass as TailwindColorKey];
		if (!colorData) return colorClass; // fallback to class name

		return colorData[format as keyof typeof colorData] || colorClass;
	};

	const [selectedFormat, setSelectedFormat] =
		useState<ColorFormat>("classname");

	// Color converter states (unified)
	const [inputColorValue, setInputColorValue] = useState("#74ACDF");
	const [outputColorFormat, setOutputColorFormat] =
		useState<ConvertibleColorFormat>("rgb");
	...
	const detectColorFormat = useCallback((colorString: string) => {
	...
	const parseCacheRef = useRef(new Map<string, RGB | null>());
	...
	const formatColor = useCallback(
		(rgb: RGB | null, format: ConvertibleColorFormat) => {
			if (!rgb) return "";
			...
			<main className="flex flex-col flex-1 gap-8 bg-background">
							...
							<div className="col-start-2 w-full flex items-center justify-start">
								<ButtonGroup className="w-full flex items-center justify-start">
									...
									<Select
										...
										onValueChange={(value) =>
											setOutputColorFormat(value as ConvertibleColorFormat)
										}
									>
				...
				<div className="flex flex-col items-end justify-start gap-4 p-6 lg:p-12 pt-4">
					<Select
						...
						onValueChange={(value) => setSelectedFormat(value as ColorFormat)}
					>
