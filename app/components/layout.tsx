...
type PropTableSection = {
	component: string;
	props: PropTableEntry[];
};

type ComponentLayoutConfig = {
	header?: {
		title: string;
		description?: React.ReactNode;
		href?: string;
	...
	content?: {
		...
		imports?: string;
		filename?: string;
		additionalCodeFiles?: GeneratedCodeFile[];
		code?: never;
	};
	installation?: {
		componentName: string;
		installDeps?: InstallationCommand[];
		dependencies?: string;
		manualFiles?: ManualFile[];
	};
	props?: PropTableSection[];
};

const ComponentConfigContext = createContext<ComponentLayoutConfig | null>(
	null,
);
...
function getElementDisplayName(element: React.ReactNode) {
...
function LayoutContent({ config }: { config: ComponentLayoutConfig }) {
	...
	const generatedCode = useMemo(() => {
		...
		const childrenJSX = reactElementToJSXString(contentData.children, {
			...
			showFunctions: true,
			functionValue: (fn: Function) => (fn.name ? fn.name : fn.toString()),
		...
		return [
			{
				filename: contentData.filename || "component.tsx",
				language: "jsx",
	...
	const activeCodeFile = codeToDisplay?.[0]?.filename;
	const [currentActiveCodeFile, setCurrentActiveCodeFile] = useState<
		string | undefined
	>(activeCodeFile);
 ...
 */
function generateInstallDeps(depsString: string): InstallationCommand[] {
 ...
 */
function extractDependenciesFromInstallDeps(
	installDeps: InstallationCommand[],
): string {
...
function InstallationGuideWrapper({
	componentName,
	installDeps,
	manualFiles,
}: {
	...
	const { value, setValue, installationTab, handleTabChange } =
		useCookiePreferences(componentName);
