...
type CodeBlockContextValue = {
	...
	data: CodeBlockDataItem[];
};

type CodeBlockRenderChild = (
	item: CodeBlockDataItem,
	index: number,
) => React.ReactNode;
...
const getCodeBlockItemValue = (item: CodeBlockDataItem) =>
	item.filename ?? item.path ?? item.language ?? "";
...
const highlight = (html: string, language?: string, themes?: CodeBlockThemes) =>
	codeToHtml(html, {
...
export const CodeBlock: (props: CodeBlockProps) => React.JSX.Element = ({
...
export const CodeBlockFiles: (
	props: Omit<React.ComponentProps<"div">, "children"> & {
		children?: CodeBlockRenderChild;
	},
) => React.JSX.Element = ({ className = "", children, ...props }) => {
	...
	return (
		<div
			...
			{...props}
		>
			{children ? data.map(children) : null}
		</div>
	);
};
...
export const CodeBlockSelectTrigger = ({
	className = "",
	...props
}: Omit<React.ComponentProps<typeof SelectTrigger>, "className"> & {
	className?: string;
}) => (
	<SelectTrigger
		className={cn(
...
) => React.JSX.Element = ({ children, className = "", ...props }) => {
	const { data } = useContext(CodeBlockContext);
...
export const CodeBlockSelectItem = ({
	className = "",
	...props
}: Omit<React.ComponentProps<typeof SelectItem>, "className"> & {
	className?: string;
...
export const CodeBlockCopyButton = ({
	...
	const code = data.find((item) => getCodeBlockItemValue(item) === value)?.code;
...
export const CodeBlockBody: (
	...
	return <div {...props}>{children ? data.map(children) : null}</div>;
};
...
export const CodeBlockContent = ({
	...
	const motionTransition: React.ComponentProps<
		typeof motion.div
	>["transition"] = isReady ? transition : { duration: 0 };
