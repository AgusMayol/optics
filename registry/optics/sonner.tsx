...
import { Info, Check, AlertCircle, AlertTriangle, Loader2 } from "lucide-react";
import { Spinner } from "@/registry/optics/spinner";
import { cn } from "@/registry/optics/lib/utils";

type ToastType = "success" | "info" | "warning" | "error" | "promise";
...
type ToastBounceDetail = {
	toastId: string;
};

type PromiseStateUpdateDetail = {
	toastId: string;
	state: "success" | "error";
	result: unknown;
};
...
declare global {
	interface WindowEventMap {
		"toast-bounce": CustomEvent<ToastBounceDetail>;
		"promise-state-update": CustomEvent<PromiseStateUpdateDetail>;
	}
}

// Toast type definitions
const TOAST_TYPES: Record<
	ToastType,
	{
...
const toastInstances = new Map<string, ToastInstance>();
// Store for managing promise instances to avoid multiple rejects
const promiseInstances = new Map<string, unknown>();
// Store for managing promise states globally
const promiseStates = new Map<string, PromiseToastState>();

/** I recommend abstracting the toast function
 *  so that you can call it without having to use toast.custom everytime. */
export function toast(toastConfig: ToastConfig = {}): ToastId {
	const {
		type = "info",
		title,
		description,
...
function PromiseToast({
	...
	const [state, setState] =
		React.useState<PromiseToastState["state"]>("loading");
	...
	React.useEffect(() => {
		const handleBounce = (event: WindowEventMap["toast-bounce"]) => {
	...
	React.useEffect(() => {
		const handleStateUpdate = (
			event: WindowEventMap["promise-state-update"],
		) => {
			if (event.detail.toastId === toastId) {
	...
	const getToastConfig = (): ToastVisualConfig => {
		switch (state) {
			case "success":
				return {
					type: "success",
			...
			case "error":
				return {
					...
					title: error,
					description:
						result instanceof Error ? result.message : "Something went wrong!",
				};
			default:
				return {
					type: "promise",
