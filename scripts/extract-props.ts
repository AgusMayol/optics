...
const baseUISourceCache = new Map<string, SourceFile>();
const baseUIPropsCache = new Map<string, ExtractedProp[]>();
const baseUIPackageJsonPath = require.resolve("@base-ui/react/package.json", {
	paths: [rootDir],
});
const baseUIPackage = JSON.parse(
	readFileSync(baseUIPackageJsonPath, "utf-8"),
) as BaseUIPackage;
...
function validateTypeSyntax(type: string): boolean {
...
function shortenLongType(type: string): string {
	if (!type) return type;
...
function extractDefaultValue(param: ParameterDeclaration): string | undefined {
...
function resolveBaseUITypesPath(moduleSpecifier: string): string | undefined {
...
function isFromBaseUIDeclaration(declaration?: Node): boolean {
	if (!declaration) return false;
	const filePath = declaration.getSourceFile().getFilePath();
	return filePath.includes(`${pathSeparator}@base-ui${pathSeparator}react`);
}

function isReactDefaultPropDeclaration(declaration?: Node): boolean {
	if (!declaration) return false;
	const filePath = declaration.getSourceFile().getFilePath();
	return filePath.includes(`${pathSeparator}@types${pathSeparator}react`);
}

function hasFilePathInType(typeText = ""): boolean {
...
function isFromComponentOrBaseUI(
	declaration?: Node,
	onlyBaseUI = false,
): boolean {
	if (!declaration) return false;
...
function extractPropsFromType(
	type: MorphType,
	{ onlyBaseUI = false }: { onlyBaseUI?: boolean } = {},
): ExtractedProp[] {
	return type.getProperties().reduce<ExtractedProp[]>((acc, symbol) => {
...
function extractBaseUIProps(
	moduleSpecifier: string,
	exportedName: string,
): ExtractedProp[] {
	const cacheKey = `${moduleSpecifier}::${exportedName}`;
	const cachedProps = baseUIPropsCache.get(cacheKey);
	...
	exported.forEach((declarations, name) => {
		...
		declarations.forEach((declaration) => {
			const typedDeclaration = declaration as Node & { getType(): MorphType };
			const type = typedDeclaration.getType();
...
function getBaseUIImports(relativePath: string): BaseUIImport[] {
	const fullPath = join(rootDir, relativePath);
	if (!existsSync(fullPath)) return [];

	const content = readFileSync(fullPath, "utf-8");
	const imports: BaseUIImport[] = [];
	const importRegex =
...
function mergeProps(
	localProps: ExtractedProp[] = [],
	externalProps: ExtractedProp[] = [],
): ExtractedProp[] {
	const merged = new Map<string, ExtractedProp>();
...
function extractComponentPropsTS(
	filePath: string,
	componentNames: string[],
): ExtractedComponentProps[] {
	...
	const results: ExtractedComponentProps[] = [];
...
function extractComponentPropsJS(
	filePath: string,
	componentNames: string[],
): ExtractedComponentProps[] {
	const fullPath = join(rootDir, filePath);
	...
	componentNames.forEach((componentName) => {
		...
		const startPos = (startMatch.index ?? 0) + startMatch[0].length;
		...
		if (paramsContent) {
			...
			params
				...
				.forEach((param) => {
					...
					let cleanDefaultValue: string | undefined = undefined;
					if (defaultValue) {
...
function getExportedComponents(filePath: string): string[] {
	const fullPath = join(rootDir, filePath);
...
function generatePropsForComponent(
	...
	let propsData: ExtractedComponentProps[] = [];
	...
	let mergedResults: ExtractedComponentProps[] = propsData.map((item) => ({
		...item,
	...
	const sourceFileName =
		componentPath
			.split("/")
			.pop()
			?.replace(/\.(jsx|tsx|js|ts)$/, "") ?? componentName;
	...
	const finalResults = mergedResults.reduce<ExtractedComponentProps[]>(
		(acc, item) => {
...
function getAllSourceFiles(dir: string, fileList: string[] = []): string[] {
	...
	files.forEach((file) => {
		...
		if (file.isDirectory()) {
			...
			// Solo incluir archivos fuente del registry excluyendo .txt y .json
			if (
 ...
 */
function scanAndExtractAllComponents(): void {
	...
	sourceFiles.forEach((fullPath) => {
		...
		// ej: registry/optics/button.tsx -> button
		...
		try {
			...
			console.error(`[props] Error processing ${relativePath}:`, message);
		}
	});
